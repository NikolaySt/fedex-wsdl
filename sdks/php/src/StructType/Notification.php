...
class Notification extends AbstractStructBase
{
    ...
    public function setSeverity($severity = null)
    {
        ...
        if (!\EnumType\NotificationSeverityType::valueIsValid($severity)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $severity, implode(', ', \EnumType\NotificationSeverityType::getValidValues())), __LINE__);
        }
    ...
    public function setMessageParameters(array $messageParameters = array())
    {
        foreach ($messageParameters as $notificationMessageParametersItem) {
            ...
            if (!$notificationMessageParametersItem instanceof \StructType\NotificationParameter) {
                throw new \InvalidArgumentException(sprintf('The MessageParameters property can only contain items of \StructType\NotificationParameter, "%s" given', is_object($notificationMessageParametersItem) ? get_class($notificationMessageParametersItem) : gettype($notificationMessageParametersItem)), __LINE__);
            }
        }
    ...
    public function addToMessageParameters(\StructType\NotificationParameter $item)
    {
        ...
        if (!$item instanceof \StructType\NotificationParameter) {
            throw new \InvalidArgumentException(sprintf('The MessageParameters property can only contain items of \StructType\NotificationParameter, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
