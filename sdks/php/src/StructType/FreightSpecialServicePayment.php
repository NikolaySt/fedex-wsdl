...
class FreightSpecialServicePayment extends AbstractStructBase
{
     ...
     * @param string $specialService
     * @return \StructType\FreightSpecialServicePayment
     */
    public function setSpecialService($specialService = null)
    {
        // validation for constraint: enumeration
        if (!\EnumType\ShipmentSpecialServiceType::valueIsValid($specialService)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $specialService, implode(', ', \EnumType\ShipmentSpecialServiceType::getValidValues())), __LINE__);
        }
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\FreightShipmentRoleType::valueIsValid($paymentType)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $paymentType, implode(', ', \EnumType\FreightShipmentRoleType::getValidValues())), __LINE__);
        }
