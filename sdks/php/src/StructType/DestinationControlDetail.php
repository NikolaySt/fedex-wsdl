...
class DestinationControlDetail extends AbstractStructBase
{
     ...
     * @uses \EnumType\DestinationControlStatementType::valueIsValid()
     * @uses \EnumType\DestinationControlStatementType::getValidValues()
     * @throws \InvalidArgumentException
     * @param string[] $statementTypes
     * @return \StructType\DestinationControlDetail
     */
    public function setStatementTypes(array $statementTypes = array())
    {
        ...
        foreach ($statementTypes as $destinationControlDetailStatementTypesItem) {
            if (!\EnumType\DestinationControlStatementType::valueIsValid($destinationControlDetailStatementTypesItem)) {
                $invalidValues[] = var_export($destinationControlDetailStatementTypesItem, true);
            }
        }
        if (!empty($invalidValues)) {
            throw new \InvalidArgumentException(sprintf('Value(s) "%s" is/are invalid, please use one of: %s', implode(', ', $invalidValues), implode(', ', \EnumType\DestinationControlStatementType::getValidValues())), __LINE__);
        }
    ...
    public function addToStatementTypes($item)
    {
        ...
        if (!\EnumType\DestinationControlStatementType::valueIsValid($item)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $item, implode(', ', \EnumType\DestinationControlStatementType::getValidValues())), __LINE__);
        }
